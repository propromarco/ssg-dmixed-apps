//
// This file was generated by the JavaTM Architecture for XML Binding(JAXB) Reference Implementation, vhudson-jaxb-ri-2.1-2 
// See <a href="http://java.sun.com/xml/jaxb">http://java.sun.com/xml/jaxb</a> 
// Any modifications to this file will be lost upon recompilation of the source schema. 
// Generated on: 2015.12.04 at 01:40:50 PM MEZ 
//


package eu.europa.publications.formex.ted.schema;

import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlAttribute;
import javax.xml.bind.annotation.XmlElement;
import javax.xml.bind.annotation.XmlRootElement;
import javax.xml.bind.annotation.XmlSchemaType;
import javax.xml.bind.annotation.XmlType;
import de.subreport.elvis.server.ws.ted.TEDFormular;
import org.jvnet.jaxb2_commons.lang.Equals;
import org.jvnet.jaxb2_commons.lang.EqualsStrategy;
import org.jvnet.jaxb2_commons.lang.HashCode;
import org.jvnet.jaxb2_commons.lang.HashCodeStrategy;
import org.jvnet.jaxb2_commons.lang.JAXBEqualsStrategy;
import org.jvnet.jaxb2_commons.lang.JAXBHashCodeStrategy;
import org.jvnet.jaxb2_commons.lang.JAXBToStringStrategy;
import org.jvnet.jaxb2_commons.lang.ToString;
import org.jvnet.jaxb2_commons.lang.ToStringStrategy;
import org.jvnet.jaxb2_commons.locator.ObjectLocator;
import org.jvnet.jaxb2_commons.locator.util.LocatorUtils;


/**
 * ROOT element F20
 * 
 * <p>Java class for F20_2014 element declaration.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;element name="F20_2014">
 *   &lt;complexType>
 *     &lt;complexContent>
 *       &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *         &lt;sequence>
 *           &lt;element name="DIRECTIVE" type="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}directive_f20"/>
 *           &lt;element name="CONTRACTING_BODY">
 *             &lt;complexType>
 *               &lt;complexContent>
 *                 &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
 *                   &lt;sequence>
 *                     &lt;element ref="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}ADDRESS_CONTRACTING_BODY"/>
 *                   &lt;/sequence>
 *                 &lt;/restriction>
 *               &lt;/complexContent>
 *             &lt;/complexType>
 *           &lt;/element>
 *           &lt;element name="OBJECT_CONTRACT" type="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}object_contract_f20"/>
 *           &lt;element name="PROCEDURE" type="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}procedure_f20" minOccurs="0"/>
 *           &lt;element name="AWARD_CONTRACT" type="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}award_contract_f20"/>
 *           &lt;element name="COMPLEMENTARY_INFO" type="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}ci_f20"/>
 *           &lt;element name="MODIFICATIONS_CONTRACT" type="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}modifications_f20"/>
 *         &lt;/sequence>
 *         &lt;attribute name="LG" use="required" type="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}t_ce_language_list" />
 *         &lt;attribute name="CATEGORY" use="required" type="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}category" />
 *         &lt;attribute name="FORM" use="required" type="{http://www.w3.org/2001/XMLSchema}anySimpleType" fixed="F20" />
 *       &lt;/restriction>
 *     &lt;/complexContent>
 *   &lt;/complexType>
 * &lt;/element>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "", propOrder = {
    "directive",
    "contractingbody",
    "objectcontract",
    "procedure",
    "awardcontract",
    "complementaryinfo",
    "modificationscontract"
})
@XmlRootElement(name = "F20_2014")
public class F202014
    implements TEDFormular, Equals, HashCode, ToString
{

    @XmlElement(name = "DIRECTIVE", required = true)
    protected DirectiveF20 directive;
    @XmlElement(name = "CONTRACTING_BODY", required = true)
    protected F202014 .CONTRACTINGBODY contractingbody;
    @XmlElement(name = "OBJECT_CONTRACT", required = true)
    protected ObjectContractF20 objectcontract;
    @XmlElement(name = "PROCEDURE")
    protected ProcedureF20 procedure;
    @XmlElement(name = "AWARD_CONTRACT", required = true)
    protected AwardContractF20 awardcontract;
    @XmlElement(name = "COMPLEMENTARY_INFO", required = true)
    protected CiF20 complementaryinfo;
    @XmlElement(name = "MODIFICATIONS_CONTRACT", required = true)
    protected ModificationsF20 modificationscontract;
    @XmlAttribute(name = "LG", required = true)
    protected TCeLanguageList lg;
    @XmlAttribute(name = "CATEGORY", required = true)
    protected Category category;
    @XmlAttribute(name = "FORM", required = true)
    @XmlSchemaType(name = "anySimpleType")
    protected String form;

    /**
     * Gets the value of the directive property.
     * 
     * @return
     *     possible object is
     *     {@link DirectiveF20 }
     *     
     */
    public DirectiveF20 getDIRECTIVE() {
        return directive;
    }

    /**
     * Sets the value of the directive property.
     * 
     * @param value
     *     allowed object is
     *     {@link DirectiveF20 }
     *     
     */
    public void setDIRECTIVE(DirectiveF20 value) {
        this.directive = value;
    }

    /**
     * Gets the value of the contractingbody property.
     * 
     * @return
     *     possible object is
     *     {@link F202014 .CONTRACTINGBODY }
     *     
     */
    public F202014 .CONTRACTINGBODY getCONTRACTINGBODY() {
        return contractingbody;
    }

    /**
     * Sets the value of the contractingbody property.
     * 
     * @param value
     *     allowed object is
     *     {@link F202014 .CONTRACTINGBODY }
     *     
     */
    public void setCONTRACTINGBODY(F202014 .CONTRACTINGBODY value) {
        this.contractingbody = value;
    }

    /**
     * Gets the value of the objectcontract property.
     * 
     * @return
     *     possible object is
     *     {@link ObjectContractF20 }
     *     
     */
    public ObjectContractF20 getOBJECTCONTRACT() {
        return objectcontract;
    }

    /**
     * Sets the value of the objectcontract property.
     * 
     * @param value
     *     allowed object is
     *     {@link ObjectContractF20 }
     *     
     */
    public void setOBJECTCONTRACT(ObjectContractF20 value) {
        this.objectcontract = value;
    }

    /**
     * Gets the value of the procedure property.
     * 
     * @return
     *     possible object is
     *     {@link ProcedureF20 }
     *     
     */
    public ProcedureF20 getPROCEDURE() {
        return procedure;
    }

    /**
     * Sets the value of the procedure property.
     * 
     * @param value
     *     allowed object is
     *     {@link ProcedureF20 }
     *     
     */
    public void setPROCEDURE(ProcedureF20 value) {
        this.procedure = value;
    }

    /**
     * Gets the value of the awardcontract property.
     * 
     * @return
     *     possible object is
     *     {@link AwardContractF20 }
     *     
     */
    public AwardContractF20 getAWARDCONTRACT() {
        return awardcontract;
    }

    /**
     * Sets the value of the awardcontract property.
     * 
     * @param value
     *     allowed object is
     *     {@link AwardContractF20 }
     *     
     */
    public void setAWARDCONTRACT(AwardContractF20 value) {
        this.awardcontract = value;
    }

    /**
     * Gets the value of the complementaryinfo property.
     * 
     * @return
     *     possible object is
     *     {@link CiF20 }
     *     
     */
    public CiF20 getCOMPLEMENTARYINFO() {
        return complementaryinfo;
    }

    /**
     * Sets the value of the complementaryinfo property.
     * 
     * @param value
     *     allowed object is
     *     {@link CiF20 }
     *     
     */
    public void setCOMPLEMENTARYINFO(CiF20 value) {
        this.complementaryinfo = value;
    }

    /**
     * Gets the value of the modificationscontract property.
     * 
     * @return
     *     possible object is
     *     {@link ModificationsF20 }
     *     
     */
    public ModificationsF20 getMODIFICATIONSCONTRACT() {
        return modificationscontract;
    }

    /**
     * Sets the value of the modificationscontract property.
     * 
     * @param value
     *     allowed object is
     *     {@link ModificationsF20 }
     *     
     */
    public void setMODIFICATIONSCONTRACT(ModificationsF20 value) {
        this.modificationscontract = value;
    }

    /**
     * Gets the value of the lg property.
     * 
     * @return
     *     possible object is
     *     {@link TCeLanguageList }
     *     
     */
    public TCeLanguageList getLG() {
        return lg;
    }

    /**
     * Sets the value of the lg property.
     * 
     * @param value
     *     allowed object is
     *     {@link TCeLanguageList }
     *     
     */
    public void setLG(TCeLanguageList value) {
        this.lg = value;
    }

    /**
     * Gets the value of the category property.
     * 
     * @return
     *     possible object is
     *     {@link Category }
     *     
     */
    public Category getCATEGORY() {
        return category;
    }

    /**
     * Sets the value of the category property.
     * 
     * @param value
     *     allowed object is
     *     {@link Category }
     *     
     */
    public void setCATEGORY(Category value) {
        this.category = value;
    }

    /**
     * Gets the value of the form property.
     * 
     * @return
     *     possible object is
     *     {@link String }
     *     
     */
    public String getFORM() {
        if (form == null) {
            return "F20";
        } else {
            return form;
        }
    }

    /**
     * Sets the value of the form property.
     * 
     * @param value
     *     allowed object is
     *     {@link String }
     *     
     */
    public void setFORM(String value) {
        this.form = value;
    }

    public boolean equals(ObjectLocator thisLocator, ObjectLocator thatLocator, Object object, EqualsStrategy strategy) {
        if (!(object instanceof F202014)) {
            return false;
        }
        if (this == object) {
            return true;
        }
        final F202014 that = ((F202014) object);
        {
            DirectiveF20 lhsDIRECTIVE;
            lhsDIRECTIVE = this.getDIRECTIVE();
            DirectiveF20 rhsDIRECTIVE;
            rhsDIRECTIVE = that.getDIRECTIVE();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "directive", lhsDIRECTIVE), LocatorUtils.property(thatLocator, "directive", rhsDIRECTIVE), lhsDIRECTIVE, rhsDIRECTIVE)) {
                return false;
            }
        }
        {
            F202014 .CONTRACTINGBODY lhsCONTRACTINGBODY;
            lhsCONTRACTINGBODY = this.getCONTRACTINGBODY();
            F202014 .CONTRACTINGBODY rhsCONTRACTINGBODY;
            rhsCONTRACTINGBODY = that.getCONTRACTINGBODY();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "contractingbody", lhsCONTRACTINGBODY), LocatorUtils.property(thatLocator, "contractingbody", rhsCONTRACTINGBODY), lhsCONTRACTINGBODY, rhsCONTRACTINGBODY)) {
                return false;
            }
        }
        {
            ObjectContractF20 lhsOBJECTCONTRACT;
            lhsOBJECTCONTRACT = this.getOBJECTCONTRACT();
            ObjectContractF20 rhsOBJECTCONTRACT;
            rhsOBJECTCONTRACT = that.getOBJECTCONTRACT();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "objectcontract", lhsOBJECTCONTRACT), LocatorUtils.property(thatLocator, "objectcontract", rhsOBJECTCONTRACT), lhsOBJECTCONTRACT, rhsOBJECTCONTRACT)) {
                return false;
            }
        }
        {
            ProcedureF20 lhsPROCEDURE;
            lhsPROCEDURE = this.getPROCEDURE();
            ProcedureF20 rhsPROCEDURE;
            rhsPROCEDURE = that.getPROCEDURE();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "procedure", lhsPROCEDURE), LocatorUtils.property(thatLocator, "procedure", rhsPROCEDURE), lhsPROCEDURE, rhsPROCEDURE)) {
                return false;
            }
        }
        {
            AwardContractF20 lhsAWARDCONTRACT;
            lhsAWARDCONTRACT = this.getAWARDCONTRACT();
            AwardContractF20 rhsAWARDCONTRACT;
            rhsAWARDCONTRACT = that.getAWARDCONTRACT();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "awardcontract", lhsAWARDCONTRACT), LocatorUtils.property(thatLocator, "awardcontract", rhsAWARDCONTRACT), lhsAWARDCONTRACT, rhsAWARDCONTRACT)) {
                return false;
            }
        }
        {
            CiF20 lhsCOMPLEMENTARYINFO;
            lhsCOMPLEMENTARYINFO = this.getCOMPLEMENTARYINFO();
            CiF20 rhsCOMPLEMENTARYINFO;
            rhsCOMPLEMENTARYINFO = that.getCOMPLEMENTARYINFO();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "complementaryinfo", lhsCOMPLEMENTARYINFO), LocatorUtils.property(thatLocator, "complementaryinfo", rhsCOMPLEMENTARYINFO), lhsCOMPLEMENTARYINFO, rhsCOMPLEMENTARYINFO)) {
                return false;
            }
        }
        {
            ModificationsF20 lhsMODIFICATIONSCONTRACT;
            lhsMODIFICATIONSCONTRACT = this.getMODIFICATIONSCONTRACT();
            ModificationsF20 rhsMODIFICATIONSCONTRACT;
            rhsMODIFICATIONSCONTRACT = that.getMODIFICATIONSCONTRACT();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "modificationscontract", lhsMODIFICATIONSCONTRACT), LocatorUtils.property(thatLocator, "modificationscontract", rhsMODIFICATIONSCONTRACT), lhsMODIFICATIONSCONTRACT, rhsMODIFICATIONSCONTRACT)) {
                return false;
            }
        }
        {
            TCeLanguageList lhsLG;
            lhsLG = this.getLG();
            TCeLanguageList rhsLG;
            rhsLG = that.getLG();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "lg", lhsLG), LocatorUtils.property(thatLocator, "lg", rhsLG), lhsLG, rhsLG)) {
                return false;
            }
        }
        {
            Category lhsCATEGORY;
            lhsCATEGORY = this.getCATEGORY();
            Category rhsCATEGORY;
            rhsCATEGORY = that.getCATEGORY();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "category", lhsCATEGORY), LocatorUtils.property(thatLocator, "category", rhsCATEGORY), lhsCATEGORY, rhsCATEGORY)) {
                return false;
            }
        }
        {
            String lhsFORM;
            lhsFORM = this.getFORM();
            String rhsFORM;
            rhsFORM = that.getFORM();
            if (!strategy.equals(LocatorUtils.property(thisLocator, "form", lhsFORM), LocatorUtils.property(thatLocator, "form", rhsFORM), lhsFORM, rhsFORM)) {
                return false;
            }
        }
        return true;
    }

    public boolean equals(Object object) {
        final EqualsStrategy strategy = JAXBEqualsStrategy.INSTANCE;
        return equals(null, null, object, strategy);
    }

    public int hashCode(ObjectLocator locator, HashCodeStrategy strategy) {
        int currentHashCode = 1;
        {
            DirectiveF20 theDIRECTIVE;
            theDIRECTIVE = this.getDIRECTIVE();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "directive", theDIRECTIVE), currentHashCode, theDIRECTIVE);
        }
        {
            F202014 .CONTRACTINGBODY theCONTRACTINGBODY;
            theCONTRACTINGBODY = this.getCONTRACTINGBODY();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "contractingbody", theCONTRACTINGBODY), currentHashCode, theCONTRACTINGBODY);
        }
        {
            ObjectContractF20 theOBJECTCONTRACT;
            theOBJECTCONTRACT = this.getOBJECTCONTRACT();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "objectcontract", theOBJECTCONTRACT), currentHashCode, theOBJECTCONTRACT);
        }
        {
            ProcedureF20 thePROCEDURE;
            thePROCEDURE = this.getPROCEDURE();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "procedure", thePROCEDURE), currentHashCode, thePROCEDURE);
        }
        {
            AwardContractF20 theAWARDCONTRACT;
            theAWARDCONTRACT = this.getAWARDCONTRACT();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "awardcontract", theAWARDCONTRACT), currentHashCode, theAWARDCONTRACT);
        }
        {
            CiF20 theCOMPLEMENTARYINFO;
            theCOMPLEMENTARYINFO = this.getCOMPLEMENTARYINFO();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "complementaryinfo", theCOMPLEMENTARYINFO), currentHashCode, theCOMPLEMENTARYINFO);
        }
        {
            ModificationsF20 theMODIFICATIONSCONTRACT;
            theMODIFICATIONSCONTRACT = this.getMODIFICATIONSCONTRACT();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "modificationscontract", theMODIFICATIONSCONTRACT), currentHashCode, theMODIFICATIONSCONTRACT);
        }
        {
            TCeLanguageList theLG;
            theLG = this.getLG();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "lg", theLG), currentHashCode, theLG);
        }
        {
            Category theCATEGORY;
            theCATEGORY = this.getCATEGORY();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "category", theCATEGORY), currentHashCode, theCATEGORY);
        }
        {
            String theFORM;
            theFORM = this.getFORM();
            currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "form", theFORM), currentHashCode, theFORM);
        }
        return currentHashCode;
    }

    public int hashCode() {
        final HashCodeStrategy strategy = JAXBHashCodeStrategy.INSTANCE;
        return this.hashCode(null, strategy);
    }

    public String toString() {
        final ToStringStrategy strategy = JAXBToStringStrategy.INSTANCE;
        final StringBuilder buffer = new StringBuilder();
        append(null, buffer, strategy);
        return buffer.toString();
    }

    public StringBuilder append(ObjectLocator locator, StringBuilder buffer, ToStringStrategy strategy) {
        strategy.appendStart(locator, this, buffer);
        appendFields(locator, buffer, strategy);
        strategy.appendEnd(locator, this, buffer);
        return buffer;
    }

    public StringBuilder appendFields(ObjectLocator locator, StringBuilder buffer, ToStringStrategy strategy) {
        {
            DirectiveF20 theDIRECTIVE;
            theDIRECTIVE = this.getDIRECTIVE();
            strategy.appendField(locator, this, "directive", buffer, theDIRECTIVE);
        }
        {
            F202014 .CONTRACTINGBODY theCONTRACTINGBODY;
            theCONTRACTINGBODY = this.getCONTRACTINGBODY();
            strategy.appendField(locator, this, "contractingbody", buffer, theCONTRACTINGBODY);
        }
        {
            ObjectContractF20 theOBJECTCONTRACT;
            theOBJECTCONTRACT = this.getOBJECTCONTRACT();
            strategy.appendField(locator, this, "objectcontract", buffer, theOBJECTCONTRACT);
        }
        {
            ProcedureF20 thePROCEDURE;
            thePROCEDURE = this.getPROCEDURE();
            strategy.appendField(locator, this, "procedure", buffer, thePROCEDURE);
        }
        {
            AwardContractF20 theAWARDCONTRACT;
            theAWARDCONTRACT = this.getAWARDCONTRACT();
            strategy.appendField(locator, this, "awardcontract", buffer, theAWARDCONTRACT);
        }
        {
            CiF20 theCOMPLEMENTARYINFO;
            theCOMPLEMENTARYINFO = this.getCOMPLEMENTARYINFO();
            strategy.appendField(locator, this, "complementaryinfo", buffer, theCOMPLEMENTARYINFO);
        }
        {
            ModificationsF20 theMODIFICATIONSCONTRACT;
            theMODIFICATIONSCONTRACT = this.getMODIFICATIONSCONTRACT();
            strategy.appendField(locator, this, "modificationscontract", buffer, theMODIFICATIONSCONTRACT);
        }
        {
            TCeLanguageList theLG;
            theLG = this.getLG();
            strategy.appendField(locator, this, "lg", buffer, theLG);
        }
        {
            Category theCATEGORY;
            theCATEGORY = this.getCATEGORY();
            strategy.appendField(locator, this, "category", buffer, theCATEGORY);
        }
        {
            String theFORM;
            theFORM = this.getFORM();
            strategy.appendField(locator, this, "form", buffer, theFORM);
        }
        return buffer;
    }


    /**
     * <p>Java class for anonymous complex type.
     * 
     * <p>The following schema fragment specifies the expected content contained within this class.
     * 
     * <pre>
     * &lt;complexType>
     *   &lt;complexContent>
     *     &lt;restriction base="{http://www.w3.org/2001/XMLSchema}anyType">
     *       &lt;sequence>
     *         &lt;element ref="{http://formex.publications.europa.eu/ted/schema/reception/R2.0.9.S01}ADDRESS_CONTRACTING_BODY"/>
     *       &lt;/sequence>
     *     &lt;/restriction>
     *   &lt;/complexContent>
     * &lt;/complexType>
     * </pre>
     * 
     * 
     */
    @XmlAccessorType(XmlAccessType.FIELD)
    @XmlType(name = "", propOrder = {
        "addresscontractingbody"
    })
    public static class CONTRACTINGBODY
        implements Equals, HashCode, ToString
    {

        @XmlElement(name = "ADDRESS_CONTRACTING_BODY", required = true)
        protected ContactContractingBody addresscontractingbody;

        /**
         * Gets the value of the addresscontractingbody property.
         * 
         * @return
         *     possible object is
         *     {@link ContactContractingBody }
         *     
         */
        public ContactContractingBody getADDRESSCONTRACTINGBODY() {
            return addresscontractingbody;
        }

        /**
         * Sets the value of the addresscontractingbody property.
         * 
         * @param value
         *     allowed object is
         *     {@link ContactContractingBody }
         *     
         */
        public void setADDRESSCONTRACTINGBODY(ContactContractingBody value) {
            this.addresscontractingbody = value;
        }

        public boolean equals(ObjectLocator thisLocator, ObjectLocator thatLocator, Object object, EqualsStrategy strategy) {
            if (!(object instanceof F202014 .CONTRACTINGBODY)) {
                return false;
            }
            if (this == object) {
                return true;
            }
            final F202014 .CONTRACTINGBODY that = ((F202014 .CONTRACTINGBODY) object);
            {
                ContactContractingBody lhsADDRESSCONTRACTINGBODY;
                lhsADDRESSCONTRACTINGBODY = this.getADDRESSCONTRACTINGBODY();
                ContactContractingBody rhsADDRESSCONTRACTINGBODY;
                rhsADDRESSCONTRACTINGBODY = that.getADDRESSCONTRACTINGBODY();
                if (!strategy.equals(LocatorUtils.property(thisLocator, "addresscontractingbody", lhsADDRESSCONTRACTINGBODY), LocatorUtils.property(thatLocator, "addresscontractingbody", rhsADDRESSCONTRACTINGBODY), lhsADDRESSCONTRACTINGBODY, rhsADDRESSCONTRACTINGBODY)) {
                    return false;
                }
            }
            return true;
        }

        public boolean equals(Object object) {
            final EqualsStrategy strategy = JAXBEqualsStrategy.INSTANCE;
            return equals(null, null, object, strategy);
        }

        public int hashCode(ObjectLocator locator, HashCodeStrategy strategy) {
            int currentHashCode = 1;
            {
                ContactContractingBody theADDRESSCONTRACTINGBODY;
                theADDRESSCONTRACTINGBODY = this.getADDRESSCONTRACTINGBODY();
                currentHashCode = strategy.hashCode(LocatorUtils.property(locator, "addresscontractingbody", theADDRESSCONTRACTINGBODY), currentHashCode, theADDRESSCONTRACTINGBODY);
            }
            return currentHashCode;
        }

        public int hashCode() {
            final HashCodeStrategy strategy = JAXBHashCodeStrategy.INSTANCE;
            return this.hashCode(null, strategy);
        }

        public String toString() {
            final ToStringStrategy strategy = JAXBToStringStrategy.INSTANCE;
            final StringBuilder buffer = new StringBuilder();
            append(null, buffer, strategy);
            return buffer.toString();
        }

        public StringBuilder append(ObjectLocator locator, StringBuilder buffer, ToStringStrategy strategy) {
            strategy.appendStart(locator, this, buffer);
            appendFields(locator, buffer, strategy);
            strategy.appendEnd(locator, this, buffer);
            return buffer;
        }

        public StringBuilder appendFields(ObjectLocator locator, StringBuilder buffer, ToStringStrategy strategy) {
            {
                ContactContractingBody theADDRESSCONTRACTINGBODY;
                theADDRESSCONTRACTINGBODY = this.getADDRESSCONTRACTINGBODY();
                strategy.appendField(locator, this, "addresscontractingbody", buffer, theADDRESSCONTRACTINGBODY);
            }
            return buffer;
        }

    }

}
